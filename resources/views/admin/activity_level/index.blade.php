...
{!! Form::open(['method' => 'get', 'route' => ['activity_level.create'], 'files'=>true]) !!}
...
<tbody>
    ...
    @foreach($activity_level as $a)

    

        <td >{!! ucwords($a->name) !!}</td>
		<td >{!! ucwords($a->detail) !!}</td>
        ...
        <td>
			{!! link_to_action('Admin\Activity_LevelController@edit',
			'Edit', array($a->id), array('class' => 'badge bg-info')) !!}

        </td>

		<td>{!! Form::open(['method' => 'POST', 'route' => ['activity_level.delete', $a->id]]) !!}
...
@section('pagination')
<span class="pagination pagination-md pull-right">{!! $activity_level->render() !!}</span>
